...
const Login = () => {
  ...
  // Submit the mobile number verification OTP
  ...
  // 3. Resend OTP for reset the password
  ...
  useEffect(() => {
    if (step === Steps.VERIFY_OTP && resendTimer === 0) {
      setResendTimer(2 * 60 * 1000);
    }
  }, [step]);

  // === FROM CURRENT VERSION - END ===

  // === FROM PREVIOUS VERSION - START ===
  ...
  // top of component
  const normalizeSLMobile = (raw) => {
    let v = (raw || "").toString().replace(/\D/g, ""); // keep digits only
    if (v.startsWith("0094")) v = v.slice(4);
    if (v.startsWith("94")) v = v.slice(2);
    if (v.length === 9 && v.startsWith("7")) v = "0" + v; // 7xxxxxxxx -> 07xxxxxxxx
    return v.slice(0, 10); // cap at 10 digits
  };
    ...
    <div className="h-screen flex overflow-hidden bg-gradient-to-br from-black via-neutral-900 to-black text-white relative">
      ...
      <div className="flex-1 flex items-center justify-center p-4 relative z-10">
        <div className="w-full max-w-md">
          ...
          {/* Step: LOGIN (From Current Version) */}
          {step === Steps.LOGIN && (
            <>
              <div className="text-center mb-10">
          ...
          {/* Step: VERIFY OTP (From Current Version) */}
            ...
            <div className="flex flex-col items-center justify-center">
              ...
              >
                ...
                <p className="text-gray-300 text-sm">
                  ⚠️ පහත විස්තර වැරදි නම්, ඔබට දැන් ඒවා වෙනස් කළ හැකිය. Verify
                  කළ පසු, මෙම විස්තර වෙනස් කළ නොහැක.
                </p>
                {/* Birth Date */}
          ...
          {/* Step: FORGOT PASSWORD (From Current Version) */}
          ...
          {/* Step: RESET_PASSWORD_OTP (From Current Version) */}
